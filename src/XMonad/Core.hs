...
getXDGDirectory xdgDir suffix =
  normalise . (</> suffix) <$>
  case xdgDir of
    XDGData   -> getEnvPath "XDG_DATA_HOME"   ".local/share"
    XDGConfig -> getEnvPath "XDG_CONFIG_HOME" ".config"
    XDGCache  -> getEnvPath "XDG_CACHE_HOME"  ".cache"
  where
    getEnvPath name fallback = do
      val <- lookupEnv name
      case val of
        Nothing -> fallback'
        Just path
